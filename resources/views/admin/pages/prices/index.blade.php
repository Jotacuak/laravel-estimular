            ...
            @foreach ($prices as $price_element)
                <div class="admin-table-elements">
                    ...
                    <div class="admin-table-element-button">
                            <button type="button" id="edit-button" data-url="{{route('prices_edit', ['rate' => $price_element->id])}}">
                    ...
                    <div class="admin-table-element-button">
                            <button type="button" id="delete-button" data-url="{{route('prices_destroy', ['rate' => $price_element->id])}}">
                                <svg viewBox="0 0 24 24">
                        ...
                        <div class="crud-form-elements">
                            ...
                            {{-- <div class="crud-form-element">
                                <select name="category_id" data-placeholder="Seleccione una categoría">
                                    <option></option>
                                    @foreach($rate)
                                        <option value="{{$rate->name}}" {{$rate->id ? 'selected':''}} class="rate_id">{{ $rate->name }}</option>
                                    @endforeach
                                </select>   
                            </div>   --}}
